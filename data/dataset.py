...
class Solution:
    def batch_loader(self, raw_dataset: str, context_length: int, batch_size: int) -> Tuple[List[List[str]], List[List[str]]]:
        ...
        idx=torch.randint(low=0, high=len(tokenized)-context_length, size=(batch_size,)).tolist()
        X=[]
        Y=[]
        for i in idx:
            X.append(tokenized[i:i+context_length])
            Y.append(tokenized[i+1:i+1+context_length])
        return X, Y
        pass
